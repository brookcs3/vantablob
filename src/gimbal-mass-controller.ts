...
function createMassMaterial() {
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      // 3-Axis Gimbal Rotation Matrices
      mat3 rotX(float a) {
      ...
      vec3 getDisplacedPosition(vec3 baseNormal, float t) {
        ...
        // Aerotrim / 3-Axis Gimbal simulation
        // 3 nested rings rotating at different, non-harmonic frequencies
        mat3 gimbal = rotZ(t * 0.45 + uSeed) * rotY(t * 0.62) * rotX(t * 0.33);
        
        // Base magnet positions on the gimbal
        vec3 baseM1 = normalize(vec3(1.0, 0.0, 0.0));
        vec3 baseM2 = normalize(vec3(-0.5, 0.866, 0.0));
        vec3 baseM3 = normalize(vec3(-0.5, -0.866, 0.0));
        vec3 baseM4 = normalize(vec3(0.0, 0.0, 1.0));
        vec3 baseM5 = normalize(vec3(0.0, 0.0, -1.0));
        
        // Apply the gimbal rotation to the magnets
        vec3 m1 = gimbal * baseM1;
        vec3 m2 = gimbal * baseM2;
        vec3 m3 = gimbal * baseM3;
        vec3 m4 = gimbal * baseM4;
        vec3 m5 = gimbal * baseM5;
        
        // Slooge Effect (Delayed Gratification)
        // Complex interference pattern that rarely peaks
        float sloogeWave = sin(t * 0.2) * cos(t * 0.31) * sin(t * 0.13);
        // smoothstep isolates only the highest peaks (top 5%)
        float slooge = smoothstep(0.9, 1.0, abs(sloogeWave)) * 2.5; 
        
        // Different strengths for each magnet
        float s1 = 1.2 + slooge; // m1 gets the slooge!
        float s2 = 0.7;
        float s3 = 0.9;
        float s4 = 0.5;
        float s5 = 1.1;
        ...
        // When slooge happens, the lobe gets wider (less sharp) and longer (higher strength)
        float sharpness1 = max(1.5, sharpness - slooge * 1.5);
        
        i1 = pow(i1, sharpness1) * s1;
        i2 = pow(i2, sharpness) * s2;
        i3 = pow(i3, sharpness) * s3;
        i4 = pow(i4, sharpness) * s4;
        i5 = pow(i5, sharpness) * s5;
...
export class GimbalMassController {
