...
export class AdvancedAudioMassController {
  ...
  // Basic Audio Setup (To be expanded)
  ...
  // Debug Toggles
  enableKick: boolean = true;
  enableSnare: boolean = true;
  enableOnset: boolean = true;
  ...
  updateAudio() {
    ...
    // Envelope following (Fast attack, slower release)
    ...
    const rawOnset = Math.min(flux / 20.0, 1.0); // Normalize flux roughly
  ...
  animate() {
    ...
    const activeOnset = this.enableOnset ? this.onset : 0;
    ...
    this.material.uniforms.uOnset.value = activeOnset;
