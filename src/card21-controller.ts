  ...
  return new THREE.ShaderMaterial({
    ...
    vertexShader: `
      ...
      void main() {
        ...
        // Audio-driven: kick → low-freq amplitude, snare → freq2 sharpening
        float noiseFreq1 = uLobeCountBias;
        float noiseFreq2 = uLobeCountBias * 2.5;

        // Agitation increases with energy, presence, and onset
        // (onset is broadband flux — feels like a "someone hit something" pulse)
        float currentAgitation = uAgitationStrength * (1.0 + uEnergy * 2.0 + uPresence + uOnset * 1.4);
        ...
        // Kick adds a very-low-freq slow swell noise layer (big slow ferrofluid heave)
        float n0 = snoise(pos * (noiseFreq1 * 0.35) + vec3(t * 0.4));

        // Combine noises for spiky/loby look
        //   n1  (big lobes) = unchanged
        //   n2  (fine noise) gets snare-multiplied → snaps sharp on snare hits
        //   n0  (sub-bass heave) scales with kick
        float edgeBoost = uEdgeRoughness * (1.0 + uSnare * 1.6);
        float displacement =
            (n1 * 0.6 + n2 * 0.4 * edgeBoost + n0 * 0.35 * uKick) * currentAgitation;

        // Directional bias (e.g., gravity or flow) — intensity slightly sharpens the pull
        displacement += pos.y * uDirectionalBias * (0.2 + uIntensity * 0.15);

        // Apply displacement along normal — baseScale slightly pumped by kick
        pos += normal * displacement * uBaseScale * (1.0 + uKick * 0.25);
...
export class Card21Controller {
  ...
  // Variant B: pipes go directly into shader uniforms — no audio springs.
  // The shader itself uses uKick/uSnare/uOnset/uIntensity as displacement multipliers.
  ...
  async enableAudio() {
    ...
    try {
      this.audioElement = new Audio('/songs/mashup24.mp3');
      ...
      console.log('[Card25] audio started — variant B (shader direct)');
    } catch (err) {
      console.error('Card25 audio playback failed', err);
    }
  }
  ...
  private animate = () => {
    ...
    // Variant B: write audio pipes directly into shader uniforms (no springs).
    // The ferrofluid shader uses these as displacement multipliers internally.
    this.material.uniforms.uKick.value = this.kick;
