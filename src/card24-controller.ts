...
export class Card24Controller {
  ...
  // Audio pipe values (0..1, peak-hold + decay)
  kick: number = 0;
  snare: number = 0;
  onset: number = 0;
  intensity: number = 0;

  // ─── Variant A: audio → spring-driven uniforms (no shader changes) ───
  // Audio pipes feed SpringValue targets. Springs smooth transitions; the
  // ferrofluid shader sees the same uniforms it always did (uBaseScale,
  // uEdgeRoughness, uAgitationStrength, uDriftSpeed) — just modulated.
  private audioSprings = {
    kick: new Spring(0, 0.25, 0.75),       // swell → baseScale
    snare: new Spring(0, 0.25, 0.75),      // edge sharpening → edgeRoughness
    onset: new Spring(0, 0.20, 0.80),      // general agitation → agitationStrength
    intensity: new Spring(0, 0.10, 0.88),  // tempo bias → driftSpeed
  };
  ...
  async enableAudio() {
    ...
    // Onset analyser (default smoothing)
    ...
    // Snare analyser (no smoothing, for sharp flux)
    ...
    try {
      ...
      console.log('[Card24] audio started — variant A (physics springs)');
    } catch (err) {
      console.error('Card24 audio playback failed', err);
    }
  }

  // Read FFT → kick / snare / onset / intensity pipes.
  // kick = sub-bass energy (0–150 Hz).
  // snare = dual-band flux × notKick gate (Card 7 pattern).
  // onset = broadband spectral flux.
  // intensity = normalized total energy.
  private updateAudio() {
  ...
  private animate = () => {
    ...
    // Read audio pipes (writes this.kick/snare/onset/intensity)
    this.updateAudio();

    // Variant A: feed pipes into audioSprings (physics-smoothed)
    this.audioSprings.kick.setTarget(this.kick);
    this.audioSprings.snare.setTarget(this.snare);
    ...
    // Variant A: audio-modulated preset uniforms (all via springs)
    ...
    this.material.uniforms.uEdgeRoughness.value = this.springs.edgeRoughness.value + this.audioSprings.snare.value * 0.55;
