...
function createMassMaterial() {
  return new THREE.ShaderMaterial({
    uniforms: {
      ...
      uVantablack: { value: 1.0 }, // Always black silhouette for this variant
      uJarvis: { value: 0.0 }
    },
    vertexShader: `
      ...
      vec3 getDisplacedPosition(vec3 baseNormal, float t) {
        ...
        // 6 Orbiting "Magnets" that pull the fluid outward (like a saw blade)
        ...
        // Sharpen the influence to create distinct lobes/spikes (saw teeth)
        float sharpness = uLobeCountBias * 1.5; // Sharper for saw blade look
        i1 = pow(i1, sharpness);
        i2 = pow(i2, sharpness);
        i3 = pow(i3, sharpness);
        i4 = pow(i4, sharpness);
        ...
        // Flatten slightly to enhance the 2D silhouette look
        pos.z *= 0.8;
...
export class HistoricMassController {
  ...
  constructor(container: HTMLElement, options: any = {}) {
    ...
    this.renderer.setClearColor(this.preset.backgroundColor, 1); // Solid background
    ...
    this.camera.position.set(0, 0, 10.8); // Straight on

    this.massGroup = new THREE.Group();
    this.scene.add(this.massGroup);

    this.material = createMassMaterial();
    this.geometry = new THREE.IcosahedronGeometry(0.68, 128); // Higher res for sharper edges
  ...
  animate() {
    ...
    this.massGroup.rotation.z = elapsed * 0.5; // Spin like a saw blade
